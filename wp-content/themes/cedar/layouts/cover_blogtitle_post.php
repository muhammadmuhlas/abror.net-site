<?php
	...
	/**
	 * COVER - BLOG TITLE POST
	 */

	if(get_theme_mod('blogcover_use_first_post', false)){
		ecko_get_first_post();
	}elseif(get_theme_mod('blogcover_use_sticky_post', false)){
...
?>
	...
	<section class="postitem posttitle center wrapper">
		<ul class="meta">
			...
			<li class="readtime"><a href="<?php the_permalink(); ?>"><?php echo ecko_estimated_reading_time(); ?> <?php esc_html_e('Min Read', 'cedar'); ?></a></li>
		...
		<a href="<?php echo esc_url(get_author_posts_url(get_the_author_meta('ID'))); ?>" class="author"><img src="https://0.gravatar.com/avatar/<?php echo esc_attr(md5(get_the_author_meta('user_email'))); ?>?s=24" alt="<?php the_author(); ?>"><?php the_author(); ?></a>
	</section>
