<?php
	...
	if(!function_exists('cedar_theme_setup')){
		function cedar_theme_setup(){
			load_theme_textdomain('cedar', get_template_directory() . '/languages');
			add_theme_support('post-formats', array('status', 'aside', 'image', 'quote', 'video', 'audio', 'link', 'gallery'));
			register_nav_menus(
				array(
					'primary' => esc_attr__('Primary Menu', 'cedar'),
					'blogheader' =>	esc_attr__('Blog Header Menu', 'cedar'),
				)
			);
		}
	}
	...
	/*-----------------------------------------------------------------------------------*/
	if(!function_exists('cedar_load_scripts')){
		function cedar_load_scripts(){
			if(!is_admin()){
				...
				wp_localize_script('ecko_js', 'ecko_theme_vars', array(
					...
					'localization_viewcomments' => esc_js('View Comments', 'cedar'),
					'localization_comments' => esc_js('Comments', 'cedar')
				));
			}
		}
	}
	...
	if(!function_exists('cedar_register_plugins')){
	...
	if(!function_exists('cedar_option_meta_box_setup')){
	...
	if(!function_exists('cedar_add_option_boxes')){
		function cedar_add_option_boxes(){
			add_meta_box(
				'cedar-post-options',
				esc_html__('Theme Post Options', 'cedar'),
	...
	if(!function_exists('cedar_options_meta_box')){
	...
	if(!function_exists('cedar_save_option_meta')){
	...
	if(!function_exists('cedar_header_post_options')){
	...
	if(!function_exists('cedar_posts_link_left_attributes')){
	...
	if(!function_exists('cedar_posts_link_right_attributes')){
	...
	if(!function_exists('cedar_get_default_widgets')){
	...
	if(!function_exists('cedar_post_loop')){
		function cedar_post_loop($query){
			if($query->is_main_query()){
				if(get_theme_mod('blogcover_use_sticky_post', false) && is_home()){
					$query->set('post__not_in', get_option('sticky_posts'));
				}elseif(get_theme_mod('blogcover_use_first_post', false) && is_home() && !is_paged()){
					$query->set('offset', 1);
				}
			}
			return $query;
	...
	add_filter('pre_get_posts', 'cedar_post_loop');


	/*-----------------------------------------------------------------------------------*/
	/* BLOG COVER - CUSTOM BACKGROUND IMAGE
	/*-----------------------------------------------------------------------------------*/

	if(!function_exists('cedar_get_blog_cover_background')){
		function cedar_get_blog_cover_background(){
			if(is_home() && !is_paged() && get_theme_mod('blogcover_use_post_background', false) && get_theme_mod('blogcover_use_first_post', false)){
				global $post;
				ecko_get_first_post();
				$background_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'background');
				$background_url = $background_url[0];
			}elseif(is_home() && !is_paged() && get_theme_mod('blogcover_use_post_background', false) && get_theme_mod('blogcover_use_sticky_post', false)){
				global $post;
				ecko_get_latest_sticky_post();
				$background_url = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'background');
				$background_url = $background_url[0];
			}else{
				$background_url = get_theme_mod('blogcover_background');
			}
	...
	if(!function_exists('cedar_category_options')){
		function cedar_category_options($tag){
			...
			<tr class="form-field">
				...
				<td>
					<input id="category-color" name="<?php echo $category_meta_title; ?>" value="<?php if(isset($category_meta)) echo esc_attr($category_meta); ?>" />
	...
	if(!function_exists('cedar_save_category_options')){
	...
	if(!function_exists('cedar_comment_format')){
				...
				<li <?php comment_class('comment'); ?> id="comment-<?php echo esc_attr(comment_ID()); ?>">
					<div class="contents">
						<a target="_blank" href="<?php comment_author_url(); ?>" class="profile">
							<img src="https://0.gravatar.com/avatar/<?php echo esc_attr(md5($comment->comment_author_email)); ?>?s=96" class="gravatar" alt="<?php comment_author(); ?>">
						</a>
						<div class="main">
							<div class="commentinfo">
								<section class="meta">
									<div class="info"><?php esc_html_e('Posted', 'cedar') ; ?> <span class="date"><?php esc_html_e('on', 'cedar') ; ?> <i class="fa fa-clock-o"></i> <time datetime="<?php comment_date('Y-m-d'); ?>"><?php comment_date(get_option('date_format')); ?></time></span></div>
								...
								<div class="buttons">
									...
									<span class="button rounded solidgray smaller isauthor"><?php esc_html_e('Author', 'cedar'); ?></span>
								</div>
							</div>
							<div class="body">
								<?php comment_text(); ?>
	...
	function cedar_customize_register($wp_customize){
		...
		$wp_customize->add_control('blogcover_disable',
			array(
				'type' => 'checkbox',
				'label' => 'Disable Blog Cover',
				'section' => 'blogcover_section',
			)
		);

		$wp_customize->add_setting('blogcover_use_first_post',
		array(
			'sanitize_callback' => 'ecko_sanitize_checkbox'
		));
		$wp_customize->add_control('blogcover_use_first_post',
			array(
				'type' => 'checkbox',
				'label' => 'Use Latest Post as Title',
				'section' => 'blogcover_section',
			)
		);
		...
		$wp_customize->add_control('blogcover_use_sticky_post',
			array(
				'type' => 'checkbox',
				'label' => 'Use Latest Sticky Post as Title',
				'section' => 'blogcover_section',
			)
		);

		$wp_customize->add_setting('blogcover_use_post_background',
		array(
			'sanitize_callback' => 'ecko_sanitize_checkbox'
		));
		$wp_customize->add_control('blogcover_use_post_background',
			array(
				'type' => 'checkbox',
				'label' => 'Use Cover Post Background Image',
				'section' => 'blogcover_section',
			)
		);
		...
		$wp_customize->add_control(
			new WP_Customize_Color_Control($wp_customize, 'color_cover_background',
				array(
					'label' => 'Blog Cover Background',
					'section' => 'colors',
